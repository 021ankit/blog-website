...
app.get("/contact-submitted", async function (req, res) {
  try {
    // Fetch all the saved contact data from your MongoDB database
    // For example, you can use the ContactUs model to retrieve all entries
    const contacts = await ContactUs.find({}).exec();

    // Render the contact-submitted.ejs page with the retrieved contact data
    res.render("contact-submitted", { contacts: contacts });
  } catch (err) {
    console.error(err);
